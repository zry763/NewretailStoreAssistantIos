...
#import "TRCTimeUtil.h"

@implementation TRCTimeUtil
...
#pragma mark 将格式为"yyyyMMdd"的日期字符串转换为"MM月dd日"格式的字符串
+ (NSString *)formatterDateMMString:(NSString *)dateString {
    ...
    return [self formatterDate:date formatString:@"MM月dd日"];
}
...
#pragma mark 将从1970开始的时间数字转换为"yyyy-MM-dd HH:mm:ss"格式的字符串
+ (NSString *)formatterDateAndTimeToStringWithTimeIntervalFrom1970:(double)timeInterval{
    
    NSTimeInterval timeData = timeInterval / 1000;
    
    NSDate *date = [NSDate dateWithTimeIntervalSince1970:timeData];
    
    return [self formatterDate:date formatString:@"yyyy-MM-dd HH:mm:ss"];
}

#pragma mark 将从1970开始的时间数字转换为"yyyy-MM-dd"格式的字符串
