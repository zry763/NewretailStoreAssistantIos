...
@interface TRCCurrencyUtil ()


@end


@implementation TRCCurrencyUtil
...
+ (NSString *)fenToYuan:(NSString *)amount {
    ...
    if (![TRCStringUtil isEmpty:amount] && [amount isPureInt]) {
...
+ (NSString *)yuanToFen:(NSString *)amount {
    ...
    if (![TRCStringUtil isEmpty:amount] && ([amount isPureInt] || [amount isPureFloat])) {
...
+ (BOOL)formatCurrency:(NSString *)string range:(NSRange)range textField:(UITextField *)textField {
    ...
    if ([@"." isEqualToString:string]) {
        ...
        if (0 == text.length || text.length == (kTRCAmountMaxLength - 1)) {
    ...
    if (NSNotFound != dotLocation) {
        ...
        if (range.location > dotLocation) {
            ...
            // 如果输入字符在小数点前面
            if (text.length == (kTRCAmountMaxLength - 1)) {
...
+ (NSString *)formatNumber:(double)number {
    ...
    return [TRCCurrencyUtil formatNumber:number formatterStyle:NSNumberFormatterCurrencyStyle];
}
